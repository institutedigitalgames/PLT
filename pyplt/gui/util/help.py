...
class MainHelpDialog(HelpDialog):
    ...
    def __init__(self, parent_window):
        ...
        intro_text = "This menu allows you to select between two modes of operation: a beginner mode, and an " \
                     "advanced mode. Both modes split up the experiment setup process into 5 steps: loading the " \
                     "data set, data pre-processing, feature selection, preference learning, and running the " \
                     "experiment.\n\n"

        beginner_heading = "Beginner Mode\n"

        beginner_text = "The beginner mode simplifies the experiment setup process such that once the data set is " \
                        "loaded, there are little-to-no parameters or options to tune. The pre-processing step " \
                        "consists of specifying whether and, if so, how many features are to be " \
                        "automatically extracted from the data set. The next steps simply " \
                        "consist of choosing whether or not to apply feature selection, choosing a preference " \
                        "learning algorithm, and finally running the experiment.\n\n"

        advanced_heading = "Advanced Mode\n"

        advanced_text = "The advanced mode on the other hand involves a more detailed setup process. This mode " \
                        "offers more functionality such as feature normalization as an added pre-processing step. " \
                        "Moreover, all but the last of the steps are encapsulated in their own detailed tab, " \
                        "each of which provides a set of options or parameters through which the experiment " \
                        "setup may be fine-tuned."
...
class LoadDataHelpDialog(HelpDialog):
    ...
    def __init__(self, parent_window):
        ...
        advanced_text = "In this advanced mode of PLT the experiment set-up process is divided into 5 steps, all but " \
                        ...
                        "pre-processing and feature selection are optional. Once all the options are set, you may " \
...
class PLHelpDialog(HelpDialog):
    ...
    def __init__(self, parent_window):
        ...
        # n.b. ensured that 'Run Experiment' button, OutputLayer#nuerons checkbox and Steps 2-5 of BeginnerMenu
...
class BeginnerStep1HelpDialog(HelpDialog):
    ...
    def __init__(self, parent_window):
        ...
        beginner_text = "This beginner mode simplifies the experiment setup process into 5 easy and quick steps: " \
                        "loading the data set, specifying whether and, if so, how many features are to be " \
                        "automatically extracted from the data set, choosing whether to apply feature selection, " \
                        "choosing a preference learning algorithm, and finally running the experiment. " \
                        "Initially, the interface areas for steps 2-5 are locked. Once a dataset is correctly " \
                        "loaded, they are enabled and accessible. The interface allows you to set up " \
                        "steps 2-4 in any order. Once all the options are set, you may run the " \
                        "experiment via the button at the bottom of the window.\n\n"
